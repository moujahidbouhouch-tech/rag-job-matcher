        ...
        for chunk in self.chunks:
            ...
            if jp:
                ...
                if (
                    posted_after
                    and jp.posted_at
                    and jp.posted_at.timestamp() < posted_after
                ):
                    continue
...
def test_search_filters_by_match_score_and_date():
    ...
    try:
        ...
        repo.insert_chunks_with_embeddings(
            [recent_chunk, old_chunk], [_vector(0.2), _vector(0.2)]
        )
...
def test_search_ranks_high_match_score_over_recency_when_similarity_equal():
    ...
    assert (
        top_doc.id == high_doc.id
    ), "High match_score job should rank above recent lower score when similarity ties"
