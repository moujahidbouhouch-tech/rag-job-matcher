...
class TraceRepo:
    ...
    def delete_document(self, document_id):
        ...
        self.job_postings = [
            jp for jp in self.job_postings if jp.document_id != document_id
        ]
    ...
    def search(
        ...
        for chunk, emb, doc in zip(self.chunks, self.embeddings, self.docs):
            ...
            results.append(
                RetrievedChunk(chunk=chunk, document=doc, job_posting=jp, score=score)
            )
...
class TraceLLM:
    ...
    def generate(
        self, prompt: str, model=None, max_tokens: int = LLM_PROVIDER_DEFAULT_MAX_TOKENS
    ) -> str:
...
def test_rag_orchestrator_ingest_calls_steps_in_order(monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_project.rag_core.ingestion.service.parse_file", fake_parse_file
    )
...
def test_rag_orchestrator_ingest_empty_document(monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_project.rag_core.ingestion.service.parse_file", lambda path: ""
    )
    monkeypatch.setattr(
        "rag_project.rag_core.ingestion.service.chunk_text",
        lambda text, max_tokens, overlap_tokens: [],
    )
    repo = TraceRepo(trace)
    embedder = TraceEmbedder(trace)
    service = IngestionService(
...
def test_rag_orchestrator_full_pipeline_integration(monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_project.rag_core.ingestion.service.chunk_text",
        lambda text, max_tokens, overlap_tokens: [text],
    )
    monkeypatch.setattr(
        "rag_project.rag_core.ingestion.service.parse_file", lambda path: str(path)
    )
    ...
    assert any(
        str(doc_ids[0]) in c.chunk_id.hex or c.document_id == doc_ids[0]
