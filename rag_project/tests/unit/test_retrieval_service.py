...
def test_retrieval_service_filters_by_doc_type_and_recency():
    recent = _chunk_with_score(
        "recent",
        0.6,
        doc_type=SUPPORTED_DOC_TYPES[2],
...
def test_retrieval_prompt_builds_context():
    ...
    rc = type(
        "RC", (), {"document": doc, "job_posting": jp, "chunk": chunk, "score": 0.9}
    )()
...
def test_retrieval_db_backed_query_returns_expected_top_k():
    ...
    chunk1 = Chunk(
        id=uuid4(), document_id=doc1.id, chunk_index=0, content="high", token_count=1
    )
    chunk2 = Chunk(
        id=uuid4(), document_id=doc2.id, chunk_index=0, content="low", token_count=1
    )
    stored = [
        _Stored(
            chunk=chunk1,
            doc=doc1,
            jp=JobPosting(document_id=doc1.id, title="Vector high"),
            score=0.9,
        ),
        _Stored(
            chunk=chunk2,
            doc=doc2,
            jp=JobPosting(document_id=doc2.id, title="Vector low"),
            score=0.1,
        ),
    ]
