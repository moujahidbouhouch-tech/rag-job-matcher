...
BOUNDARY_SIGNALS: dict[str, tuple[re.Pattern, float]] = {
    name: (
        re.compile(CHUNK_BOUNDARY_PATTERNS[name]),
        BOUNDARY_PATTERN_WEIGHTS.get(name, 0.0),
    )
...
def _is_code_line(line: str) -> bool:
    ...
    if l.startswith(
        ("def ", "class ", "import ", "from ", "return ", "print(", "logger.", "@")
    ):
        return True
    ...
    if line.startswith((" ", "\t")):
        if l.startswith(
            ("if ", "for ", "while ", "try:", "except:", "else:", "elif ", "with ")
        ):
            return True
...
def _best_signal_score(line: str) -> float:
    scores = [
        score for pattern, score in BOUNDARY_SIGNALS.values() if pattern.search(line)
    ]
...
def _find_all_priority_boundaries(
    text: str, min_pos: int, target_pos: int
    ...
    for priority, pattern in patterns.items():
        for match in re.finditer(pattern, text):
            pos = (
                match.end()
                if priority == BoundaryPriority.PARAGRAPH_BREAK
                else match.start()
            )
...
def _find_safe_split_point(
    text: str, target_pos: int, min_pos: int, proximity_weight: float
) -> int:
    ...
    def weighted_score(c: BoundaryCandidate) -> float:
        ...
        return (
            1 - proximity_weight
        ) * priority_score + proximity_weight * distance_score
...
def _split_by_boundaries(
    lines: Sequence[str], boundaries: Sequence[Boundary]
) -> List[str]:
    if not boundaries:
        return ["\n".join(lines).strip()]
...
def _enforce_max_size(
    chunks: List[str], max_words: int, overlap_words: int
) -> List[str]:
    ...
    for chunk in chunks:
        ...
        if len(words) > max_words * STRUCTURED_OVERSIZE_FACTOR:
            final_chunks.extend(
                chunk_text(chunk, max_tokens=max_words, overlap_tokens=overlap_words)
            )
        else:
...
def _llm_boundaries(
    ...
    try:
        ...
        validated = sorted(
            {b.index: b for b in validated}.values(), key=lambda b: b.index
        )
        return validated
    except Exception:
        return []
...
def _split_to_size(chunks: List[str], config: ChunkConfig) -> List[str]:
        ...
        while True:
            ...
            min_idx = min(
                ...
                max(
                    ...
                    int(config.max_chunk_words * MIN_SPLIT_RATIO),
                ),
            )
            min_pos = positions[min_idx]
            split_pos = _find_safe_split_point(
                remaining, target_pos, min_pos, config.proximity_weight
            )
            ...
            if split_pos <= 0 or split_pos >= len(remaining) - 1:
                ...
                fallback_chunks = chunk_text(
                    remaining, max_tokens=config.max_chunk_words, overlap_tokens=0
                )
    ...
    for item in sized:
        if len(item.split()) > config.max_chunk_words_hard:
            ...
            forced.extend(
                chunk_text(item, max_tokens=config.max_chunk_words, overlap_tokens=0)
            )
        else:
            forced.append(item)
    if logger.isEnabledFor(logging.DEBUG):
...
def chunk_structured(
    ...
    segments = _segment_large_text(
        text,
        config.max_llm_input_words if config.use_llm else config.max_chunk_words * 3,
    )
    all_chunks: List[str] = []
    for segment in segments:
        ...
        if (
            config.use_llm
            and llm_generate
            and len(segment.split()) <= config.max_llm_input_words
        ):
        ...
        for chunk in overlapped:
            if (
                merged
                and len(chunk.split()) < config.min_chunk_words
                and len(merged[-1].split()) < config.min_chunk_words
            ):
                merged[-1] = merged[-1] + "\n\n" + chunk
            else:
                merged.append(chunk)
        ...
        logger.info(
            "Segment produced %s chunks (sizes=%s)",
            len(merged),
            [len(c.split()) for c in merged],
        )
    return all_chunks
