...
class DomainExtractionService:
    ...
    def __init__(self, llm: LLMProvider, embedder: EmbeddingProvider, chunk_repo: ChunkRepository):
        self.llm = llm
    ...
    def extract_domain_mappings(self, job_text: str) -> DomainMapping:
        ...
        logger.debug("Domain extraction start: job_text_len=%d candidate_summary_len=%d", len(job_text), len(candidate_summary))
        prompt = DOMAIN_MAPPING_EXTRACTION_PROMPT.format(job_text=job_text, candidate_summary=candidate_summary)
        try:
            raw = self.llm.generate(prompt, max_tokens=DOMAIN_MAPPING_MAX_TOKENS)
            cleaned = self._strip_code_fence(raw)
            data = json.loads(cleaned)
            logger.info("Domain extraction succeeded with %d language mappings", len(data.get("language_mappings", [])))
            ...
            logger.error("Domain extraction failed, returning empty mappings: %s", exc, exc_info=True)
            # Fallback to empty mappings on any parse/LLM error
            return DomainMapping(language_mappings=[], skill_demonstrations=[], credential_mappings=[])

    def _candidate_summary(self) -> str:
        query_embedding = self.embedder.embed(["candidate profile education experience skills"])[0]
