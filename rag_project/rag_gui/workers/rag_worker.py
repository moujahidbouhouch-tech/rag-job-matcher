...
class JobLoaderWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            jobs = []
            with psycopg.connect(connect_timeout=GUI_JOBLOADER_DB_TIMEOUT, **self.conn_settings) as conn:
...
class RAGQueryWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            logger.info("RAGQueryWorker starting question len=%d selected_ids=%d", len(self._question), len(self._selected_ids))
