...
def main():
    ...
    parser.add_argument("--job-file", required=True, help="Path to job description text file")
    ...
    logger.info("Benchmark completed in %.2fs match_rate=%.1f", elapsed, result.match_rate)

    print(f"Elapsed: {elapsed:.2f}s")
    print(f"Match rate: {result.match_rate:.1f}% ({result.match_count}/{len(result.evaluations)})")
